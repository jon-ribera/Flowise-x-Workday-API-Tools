...
const asOfDate = typeof $asOfDate !== 'undefined' ? $asOfDate : null;
const country = typeof $country !== 'undefined' ? $country : null;
const includeLocalTaxAuthorities = typeof $includeLocalTaxAuthorities !== 'undefined' ? $includeLocalTaxAuthorities : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const taxAuthority = typeof $taxAuthority !== 'undefined' ? $taxAuthority : null;

const apiPath = '/payroll/v2/minimumWageRates';
const qParts = [];
(function buildQuery(params) {
    if (asOfDate !== null && asOfDate !== undefined) params.push(`asOfDate=${encodeURIComponent(asOfDate)}`);
    if (country !== null && country !== undefined) params.push(`country=${encodeURIComponent(country)}`);
    if (includeLocalTaxAuthorities !== null && includeLocalTaxAuthorities !== undefined) params.push(`includeLocalTaxAuthorities=${encodeURIComponent(includeLocalTaxAuthorities)}`);
    ...
    if (taxAuthority !== null && taxAuthority !== undefined) params.push(`taxAuthority=${encodeURIComponent(taxAuthority)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the Payroll domain.",
