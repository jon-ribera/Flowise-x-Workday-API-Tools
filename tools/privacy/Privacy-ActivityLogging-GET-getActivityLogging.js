...
const from = typeof $from !== 'undefined' ? $from : null;
const instancesReturned = typeof $instancesReturned !== 'undefined' ? $instancesReturned : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const returnUserActivityEntryCount = typeof $returnUserActivityEntryCount !== 'undefined' ? $returnUserActivityEntryCount : null;
const systemAccount = typeof $systemAccount !== 'undefined' ? $systemAccount : null;
const taskId = typeof $taskId !== 'undefined' ? $taskId : null;
const to = typeof $to !== 'undefined' ? $to : null;

const apiPath = '/privacy/v1/activityLogging';
const qParts = [];
(function buildQuery(params) {
    if (from !== null && from !== undefined) params.push(`from=${encodeURIComponent(from)}`);
    if (instancesReturned !== null && instancesReturned !== undefined) params.push(`instancesReturned=${encodeURIComponent(instancesReturned)}`);
    ...
    if (returnUserActivityEntryCount !== null && returnUserActivityEntryCount !== undefined) params.push(`returnUserActivityEntryCount=${encodeURIComponent(returnUserActivityEntryCount)}`);
    if (systemAccount !== null && systemAccount !== undefined) params.push(`systemAccount=${encodeURIComponent(systemAccount)}`);
    if (taskId !== null && taskId !== undefined) params.push(`taskId=${encodeURIComponent(taskId)}`);
    if (to !== null && to !== undefined) params.push(`to=${encodeURIComponent(to)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the Privacy domain.",
