...
const customObjectAlias = typeof $customObjectAlias !== 'undefined' ? $customObjectAlias : null;
const updateIfExists = typeof $updateIfExists !== 'undefined' ? $updateIfExists : null;
const bulk = typeof $bulk !== 'undefined' ? $bulk : null;
const requestBody = typeof $requestBody !== 'undefined' ? $requestBody : null;

const apiPath = `/customObject/v2/customObjects/${customObjectAlias}`;
const qParts = [];
(function buildQuery(params) {
    if (updateIfExists !== null && updateIfExists !== undefined) params.push(`updateIfExists=${encodeURIComponent(updateIfExists)}`);
    if (bulk !== null && bulk !== undefined) params.push(`bulk=${encodeURIComponent(bulk)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the CustomObjectDataSingleInstance domain.",
