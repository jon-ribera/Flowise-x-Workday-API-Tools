...
const name = typeof $name !== 'undefined' ? $name : null;
const notificationType = typeof $notificationType !== 'undefined' ? $notificationType : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;

const apiPath = '/connect/v2/messageTemplates';
const qParts = [];
(function buildQuery(params) {
    ...
    if (name !== null && name !== undefined) params.push(`name=${encodeURIComponent(name)}`);
    if (notificationType !== null && notificationType !== undefined) params.push(`notificationType=${encodeURIComponent(notificationType)}`);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the Connect domain.",
