...
const addressConfigurationFormat = typeof $addressConfigurationFormat !== 'undefined' ? $addressConfigurationFormat : null;
const currentAddress = typeof $currentAddress !== 'undefined' ? $currentAddress : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const useWesternScript = typeof $useWesternScript !== 'undefined' ? $useWesternScript : null;

const apiPath = `/person/v4/countries/${ID}/addressComponents`;
const qParts = [];
(function buildQuery(params) {
    if (addressConfigurationFormat !== null && addressConfigurationFormat !== undefined) params.push(`addressConfigurationFormat=${encodeURIComponent(addressConfigurationFormat)}`);
    if (currentAddress !== null && currentAddress !== undefined) params.push(`currentAddress=${encodeURIComponent(currentAddress)}`);
    ...
    if (useWesternScript !== null && useWesternScript !== undefined) params.push(`useWesternScript=${encodeURIComponent(useWesternScript)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the Person domain.",
