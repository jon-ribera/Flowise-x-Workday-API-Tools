...
const filterBy = typeof $filterBy !== 'undefined' ? $filterBy : null;
const filterCondition = typeof $filterCondition !== 'undefined' ? $filterCondition : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const parent = typeof $parent !== 'undefined' ? $parent : null;
const project = typeof $project !== 'undefined' ? $project : null;
const topLevelPhase = typeof $topLevelPhase !== 'undefined' ? $topLevelPhase : null;

const apiPath = '/projects/v1/planPhases';
const qParts = [];
(function buildQuery(params) {
    if (filterBy !== null && filterBy !== undefined) params.push(`filterBy=${encodeURIComponent(filterBy)}`);
    if (filterCondition !== null && filterCondition !== undefined) params.push(`filterCondition=${encodeURIComponent(filterCondition)}`);
    ...
    if (parent !== null && parent !== undefined) params.push(`parent=${encodeURIComponent(parent)}`);
    if (project !== null && project !== undefined) params.push(`project=${encodeURIComponent(project)}`);
    if (topLevelPhase !== null && topLevelPhase !== undefined) params.push(`topLevelPhase=${encodeURIComponent(topLevelPhase)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the Projects domain.",
