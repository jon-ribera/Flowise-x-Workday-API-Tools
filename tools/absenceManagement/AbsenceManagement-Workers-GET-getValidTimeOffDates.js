...
const date = typeof $date !== 'undefined' ? $date : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const position = typeof $position !== 'undefined' ? $position : null;
const timeOff = typeof $timeOff !== 'undefined' ? $timeOff : null;

const apiPath = `/absenceManagement/v4/workers/${ID}/validTimeOffDates`;
const qParts = [];
(function buildQuery(params) {
    if (date !== null && date !== undefined) params.push(`date=${encodeURIComponent(date)}`);
    ...
    if (position !== null && position !== undefined) params.push(`position=${encodeURIComponent(position)}`);
    if (timeOff !== null && timeOff !== undefined) params.push(`timeOff=${encodeURIComponent(timeOff)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the AbsenceManagement domain.",
