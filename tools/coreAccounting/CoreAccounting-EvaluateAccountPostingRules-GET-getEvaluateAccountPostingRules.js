...
const accountPostingRuleSet = typeof $accountPostingRuleSet !== 'undefined' ? $accountPostingRuleSet : null;
const accountPostingRuleType = typeof $accountPostingRuleType !== 'undefined' ? $accountPostingRuleType : null;
const dimensionValue = typeof $dimensionValue !== 'undefined' ? $dimensionValue : null;

const apiPath = '/coreAccounting/v1/evaluateAccountPostingRules';
const qParts = [];
(function buildQuery(params) {
    if (accountPostingRuleSet !== null && accountPostingRuleSet !== undefined) params.push(`accountPostingRuleSet=${encodeURIComponent(accountPostingRuleSet)}`);
    if (accountPostingRuleType !== null && accountPostingRuleType !== undefined) params.push(`accountPostingRuleType=${encodeURIComponent(accountPostingRuleType)}`);
    if (dimensionValue !== null && dimensionValue !== undefined) params.push(`dimensionValue=${encodeURIComponent(dimensionValue)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the CoreAccounting domain.",
