...
const desc = typeof $desc !== 'undefined' ? $desc : null;
const flag = typeof $flag !== 'undefined' ? $flag : null;
const label = typeof $label !== 'undefined' ? $label : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const myCases = typeof $myCases !== 'undefined' ? $myCases : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const openCases = typeof $openCases !== 'undefined' ? $openCases : null;
const sort = typeof $sort !== 'undefined' ? $sort : null;
const status = typeof $status !== 'undefined' ? $status : null;
const substatus = typeof $substatus !== 'undefined' ? $substatus : null;
const team = typeof $team !== 'undefined' ? $team : null;

const apiPath = '/helpCase/v4/cases';
const qParts = [];
(function buildQuery(params) {
    if (desc !== null && desc !== undefined) params.push(`desc=${encodeURIComponent(desc)}`);
    if (flag !== null && flag !== undefined) params.push(`flag=${encodeURIComponent(flag)}`);
    if (label !== null && label !== undefined) params.push(`label=${encodeURIComponent(label)}`);
    if (limit !== null && limit !== undefined) params.push(`limit=${encodeURIComponent(limit)}`);
    if (myCases !== null && myCases !== undefined) params.push(`myCases=${encodeURIComponent(myCases)}`);
    if (offset !== null && offset !== undefined) params.push(`offset=${encodeURIComponent(offset)}`);
    if (openCases !== null && openCases !== undefined) params.push(`openCases=${encodeURIComponent(openCases)}`);
    if (sort !== null && sort !== undefined) params.push(`sort=${encodeURIComponent(sort)}`);
    if (status !== null && status !== undefined) params.push(`status=${encodeURIComponent(status)}`);
    if (substatus !== null && substatus !== undefined) params.push(`substatus=${encodeURIComponent(substatus)}`);
    if (team !== null && team !== undefined) params.push(`team=${encodeURIComponent(team)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the HelpCase domain.",
