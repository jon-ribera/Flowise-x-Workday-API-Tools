...
const enrollmentOfferingTypeWID = typeof $enrollmentOfferingTypeWID !== 'undefined' ? $enrollmentOfferingTypeWID : null;
const onlyCurrentElections = typeof $onlyCurrentElections !== 'undefined' ? $onlyCurrentElections : null;
const planType = typeof $planType !== 'undefined' ? $planType : null;

const apiPath = '/benefitEnrollmentEventOfferings/v1/employeeEnrollmentEvent';
const qParts = [];
(function buildQuery(params) {
    if (enrollmentOfferingTypeWID !== null && enrollmentOfferingTypeWID !== undefined) params.push(`enrollmentOfferingTypeWID=${encodeURIComponent(enrollmentOfferingTypeWID)}`);
    if (onlyCurrentElections !== null && onlyCurrentElections !== undefined) params.push(`onlyCurrentElections=${encodeURIComponent(onlyCurrentElections)}`);
    if (planType !== null && planType !== undefined) params.push(`planType=${encodeURIComponent(planType)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the BenefitEnrollmentEventOfferings domain.",
