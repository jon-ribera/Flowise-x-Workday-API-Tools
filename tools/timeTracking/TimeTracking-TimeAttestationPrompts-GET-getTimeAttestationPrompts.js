...
const dateTime = typeof $dateTime !== 'undefined' ? $dateTime : null;
const eventType = typeof $eventType !== 'undefined' ? $eventType : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;
const timeZone = typeof $timeZone !== 'undefined' ? $timeZone : null;
const worker = typeof $worker !== 'undefined' ? $worker : null;

const apiPath = '/timeTracking/v5/timeAttestationPrompts';
const qParts = [];
(function buildQuery(params) {
    if (dateTime !== null && dateTime !== undefined) params.push(`dateTime=${encodeURIComponent(dateTime)}`);
    if (eventType !== null && eventType !== undefined) params.push(`eventType=${encodeURIComponent(eventType)}`);
    ...
    if (timeZone !== null && timeZone !== undefined) params.push(`timeZone=${encodeURIComponent(timeZone)}`);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the TimeTracking domain.",
