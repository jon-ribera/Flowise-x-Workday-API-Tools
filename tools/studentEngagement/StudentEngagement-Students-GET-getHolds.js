...
const holdType = typeof $holdType !== 'undefined' ? $holdType : null;
const reason = typeof $reason !== 'undefined' ? $reason : null;
const showInactive = typeof $showInactive !== 'undefined' ? $showInactive : null;

const apiPath = `/studentEngagement/v1/students/${ID}/holds`;
const qParts = [];
(function buildQuery(params) {
    if (holdType !== null && holdType !== undefined) params.push(`holdType=${encodeURIComponent(holdType)}`);
    if (reason !== null && reason !== undefined) params.push(`reason=${encodeURIComponent(reason)}`);
    if (showInactive !== null && showInactive !== undefined) params.push(`showInactive=${encodeURIComponent(showInactive)}`);
})(qParts);
