...
const periodEndDate = typeof $periodEndDate !== 'undefined' ? $periodEndDate : null;
const showMostRecentOnly = typeof $showMostRecentOnly !== 'undefined' ? $showMostRecentOnly : null;

const apiPath = `/globalPayroll/v1/payGroups/${ID}/periods`;
const qParts = [];
(function buildQuery(params) {
    ...
    if (periodEndDate !== null && periodEndDate !== undefined) params.push(`periodEndDate=${encodeURIComponent(periodEndDate)}`);
    if (showMostRecentOnly !== null && showMostRecentOnly !== undefined) params.push(`showMostRecentOnly=${encodeURIComponent(showMostRecentOnly)}`);
})(qParts);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the GlobalPayroll domain.",
