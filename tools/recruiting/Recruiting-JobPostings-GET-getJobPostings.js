...
const category = typeof $category !== 'undefined' ? $category : null;
const jobPosting = typeof $jobPosting !== 'undefined' ? $jobPosting : null;
const jobRequisition = typeof $jobRequisition !== 'undefined' ? $jobRequisition : null;
const jobSite = typeof $jobSite !== 'undefined' ? $jobSite : null;
const limit = typeof $limit !== 'undefined' ? $limit : null;
const offset = typeof $offset !== 'undefined' ? $offset : null;

const apiPath = '/recruiting/v4/jobPostings';
const qParts = [];
(function buildQuery(params) {
    if (category !== null && category !== undefined) params.push(`category=${encodeURIComponent(category)}`);
    if (jobPosting !== null && jobPosting !== undefined) params.push(`jobPosting=${encodeURIComponent(jobPosting)}`);
    if (jobRequisition !== null && jobRequisition !== undefined) params.push(`jobRequisition=${encodeURIComponent(jobRequisition)}`);
    if (jobSite !== null && jobSite !== undefined) params.push(`jobSite=${encodeURIComponent(jobSite)}`);
...
const ERROR_MAP = {
    ...
    403: "Forbidden — The Integration System User (ISU) lacks required domain security policies. Verify the Security Group has permissions on the Recruiting domain.",
